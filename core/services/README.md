...
(extract/score/classify IOCs from evidence), `report_service.py`
(generate/fetch reports).

**Responsibility:** Translates a frontend request into calls against
`core/graph`, `core/db`, and `core/reporting` — and nothing else, with two
...
and `docs/dependency-rules.md` rule 4a), and `threat_intel_service.py` calls
...
`docs/dependency-rules.md` rule 4b). Both also call `core/memory` (the same
...
`evidence_service.py`'s `notify_memory` pipeline stage and again by
`threat_intel_service.py`'s). This is the one place business rules that span
multiple subsystems are coordinated.
