"""extend timeline_event_type_enum for sast_assessed

Revision ID: 27d5a3474dca
Revises: eea88afcd84d
...
revision: str = '27d5a3474dca'
down_revision: str | None = 'eea88afcd84d'
...
_ORIGINAL_EVENT_TYPE_VALUES = (
    ...
    'linux_advisory_assessed', 'owasp_web_assessed',
)
_NEW_EVENT_TYPE_VALUES = ('sast_assessed',)
_ALL_EVENT_TYPE_VALUES = _ORIGINAL_EVENT_TYPE_VALUES + _NEW_EVENT_TYPE_VALUES


def upgrade() -> None:
    """docs/adr/0021 (OWASP Security Agent / AST SAST): extend
    `timeline_event_type_enum` additively with `sast_assessed` — the prior
    twelve values are never renamed/removed. Dialect branching mirrors
    `eea88afcd84d`'s identical, already-established pattern."""
    bind = op.get_bind()
...
def downgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        ...
        # intentionally not attempted here, mirroring `eea88afcd84d`'s
