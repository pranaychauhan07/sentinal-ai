"""Unit tests for core/owasp_web/rule_engine.py — the generic, data-driven
RuleEngine that's this package's extensibility seam."""
...
from core.owasp_web.rule_engine import Matcher, Rule, RuleEngine, register_callable
...
def test_regex_rule_matches() -> None:
    ...
    assert matches[0].category == OwaspCategory.A05_SECURITY_MISCONFIGURATION
...
def test_literal_substring_rule() -> None:
    rule = _regex_rule(
        id="literal",
        matcher=Matcher(kind=MatcherKind.LITERAL_SUBSTRING, pattern="unsafe-inline"),
    )
    engine = RuleEngine([rule])
    matches = engine.evaluate("script-src UNSAFE-INLINE (case-insensitive)")
    assert len(matches) == 1


def test_callable_signature_rule() -> None:
    register_callable(
        "owasp_test_always_true", lambda text: "matched!" if "trigger" in text else None
    )
    rule = _regex_rule(
        id="callable_rule",
        matcher=Matcher(
            kind=MatcherKind.CALLABLE_SIGNATURE, callable_name="owasp_test_always_true"
        ),
    )
    engine = RuleEngine([rule])
